...
/**
 * Markdown 表格是「裸」的 `<table>`，外面没有可以挂横向滚动的容器。这里给每张表套两层：
 *
 *     div.table-wrap     边框、圆角、定位上下文；不滚动，负责裁圆角和挂「可横滑」提示
 *       └ div.table-scroll   overflow-x: auto，横向滚动发生在这一层
 *           └ table
 *
 * 为什么要两层：提示气泡要固定在表格右缘、不跟着内容滚走，就得相对不滚动的外层定位；
 * 而滚动容器自己又得裁剪内容，两者放在同一个元素上做不到（滚动元素的绝对定位子元素
 * 会跟着内容一起滚）。
 *
 * 不能像以前那样直接在 `table` 上写 `display: block; overflow-x: auto` —— 那样
 * `thead` / `tbody` 会各自被包进一个匿名 table，两张表分别算列宽，表头和数据列就对不齐了；
 * `border-collapse` 也会跟着失效。
 */
function rehypeWrapTables() {
    return (tree) => {
        visit(tree, 'element', (node, index, parent) => {
            ...
            const parentClass = parent.properties && parent.properties.className;
            if (Array.isArray(parentClass) && parentClass.includes('table-wrap')) return;
...
/**
 * 文章里的「问答实战」是一题一答的长列表（题目 / 提问意图 / 推荐回答 / 追问准备）。答案全摊在
 * 屏幕上，想先自测就没法自测了。这里把每道题的回答收进 `<details>`，默认折叠、点一下展开；
 * 题目本身（标题，或 `**Q：…**` 那一行）始终可见，所以目录锚点和分享链接都不受影响。
 *
 * 判定只看结构，不写死到某一篇文章里：
 *   - 只在「问答实战」小节里生效（`## 问答实战`、`### 问答实战（精讲及回答）` 都算），小节遇到
 *     同级或更浅的标题就结束。
 *   - 题目是比小节更深的标题，或者整段只有加粗的 `**Q：…**`。
 *   - 小节里的分组标题（如 `### 索引与 B+ 树`）后面紧跟的就是题目，它自己不算题目，保持原样。
 *   - 回答是题目之后直到下一题 / 下一个标题 / `---` 为止的内容；里面必须真的有正文（段落或
 *     引用）才折叠 —— 只跟着表格、图示的标题（如 `### 速查表：考前扫一眼`）不是题目。
 */
function rehypeCollapseQaAnswers() {
    const SECTION = /问答实战/;
    const QUESTION = /^Q\s*[：:]/;
    ...
    /** 整段只有一个加粗，也就是 `**Q：…**` 这种独占一行的题目。 */
    const isBoldLine = (node) => {
    ...
    const isQuestion = (node) =>
        headingLevel(node) > 0 || (isBoldLine(node) && QUESTION.test(hastToString(node).trim()));

    const isProse = (node) =>
        node.type === 'element' && (node.tagName === 'p' || node.tagName === 'blockquote');

    const nextMeaningful = (list, from) => {
        for (let index = from; index < list.length; index += 1) {
            if (!isBlank(list[index])) return list[index];
        }
        return null;
    };
    ...
    return (tree) => {
        ...
        for (let index = 0; index < children.length; index += 1) {
            ...
            if (level) {
                if (SECTION.test(hastToString(node))) {
                    sectionDepth = level;
                    continue;
                }
                if (sectionDepth && level <= sectionDepth) sectionDepth = 0;
            }

            if (!sectionDepth) continue;
            if (!isQuestion(node)) continue;
            // 分组标题：后面跟着的就是题目，它自己不是题目。
            if (level && isQuestion(nextMeaningful(children, index + 1))) continue;

            let cursor = index + 1;
            for (; cursor < children.length; cursor += 1) {
                const next = children[cursor];
                if (next.type === 'element' && (next.tagName === 'hr' || isQuestion(next))) break;
            }

            const run = children.slice(index + 1, cursor);
            while (run.length && isBlank(run[0])) run.shift();
            while (run.length && isBlank(run[run.length - 1])) run.pop();

            // 后面只有表格 / 图示的标题不是题目，原样留着。
            if (!run.some(isProse)) continue;
                ...
                children: [
                    {
                        ...
                        tagName: 'summary',
                        properties: {},
                        children: [{ type: 'text', value: '查看回答' }],
                    },
                    {
                        type: 'element',
                        tagName: 'div',
                        properties: { className: ['qa-answer-body'] },
                        children: run,
                    },
                ],
            });
