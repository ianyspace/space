...
多篇文章都要用的组件放到 `content/components/`，写法一样。（目前只有一个 `term`，见下）

（新增或删掉组件文件后，重启一次 `npm run dev`。）

### 术语卡（`<term>`）

专业名词可以标成术语卡：PC 上鼠标浮上去、手机上点一下，弹出一句话解释。术语在正文里是一条**虚线**
（链接是实线），扫读时不会和链接混起来。
...
临时解释（不进术语库）用 `<term def="…">`。

解释集中在 `content/glossary/`，按领域分文件（现在是 `jvm.js`），在 `index.js` 里挂上就能用。
词条写成 `{ def: '一句话解释', more: '可选的一行补充' }`，两者都按**纯文本**渲染（别写反引号、星号）。
查找不区分大小写和空格，所以 `GC Roots` / `gc roots` 命中同一条。

两条约定：

- **只收正文自己没解释过的词**。有专节、有就地一句说明、或者在表格里给了优缺点的，都不要标 ——
  否则点开看到的还是刚读过的内容。
- **同一个词条在正文里每次出现都标**，不要只标第一次 —— 读者从中间读进来也要能点开。
  同一行内重复出现的同一个词标一次就够。不要标进标题、代码块、行内代码、表格、
  文章开头的导读段和文末的速查总表。另外注意同形异义：一个词在这篇里是别的意思
  （比如「区分度」指考点区分度而不是索引区分度），那种地方别标。

术语库里没有的词不会让构建失败，只在开发时打一条 warning，并降级成普通文字（不画虚线 ——
画了线点开没反应比不标更糟）。
